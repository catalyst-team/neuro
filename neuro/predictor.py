...
from neuro.coords_generator import CoordsGenerator


class Predictor:
    """Only useful for serving... other methods would use a dataloader"""

    def __init__(self, model, volume_shape, subvolume_shape, n_subvolumes, n_classes):
        """Docs."""
        self.model = model
        ...
        self.generator = CoordsGenerator(self.volume_shape, self.subvolume_shape)
    ...
    def predict(self, image_path):
        ...
        one_hot_predicted_segmentation = torch.zeros(
            tuple(np.insert(self.volume_shape, 0, self.n_classes)), dtype=torch.uint8,
        ).to(self.device)
        ...
        with torch.no_grad():
            for coords in coords_list:
                ...
                _, predicted = torch.max(
                    torch.nn.functional.log_softmax(self.model(torch_slice), dim=1), 1,
                )
            ...
            predicted_segmentation = torch.max(one_hot_predicted_segmentation, 0)[1]
